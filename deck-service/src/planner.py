...
LAYOUT_USAGE = {
    ...
    "harvey_ball":        "A Harvey-ball rating grid — PREFER this over a plain table when comparing 3 or more OPTIONS across several qualitative CRITERIA by relative strength (not exact numbers). Emit `title`, `options` (2 to 4 column headers) and `criteria`: 2 to 6 objects each with a `label` and `scores` (one integer 0 to 4 per option, 0 = empty circle, 4 = full).",
