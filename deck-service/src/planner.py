...
- CLAIM STRENGTH is a claim like any other, and is checked. A figure being right does not make the
  sentence around it right. Specifically:
  * SAFETY. Never state that adverse events, side effects or safety concerns were absent unless the
    source says so IN THOSE TERMS, and never generalise one trial's safety statement across several
    trials. A trial that did not report adverse events is not evidence that there were none. If the
    source says none were TREATMENT RELATED, say exactly that, not that there were none.
  * SIZE OF EFFECT. Use the authors' own characterisation. If a paper calls a result "modest", or
    reports it below a minimal clinically important difference, then it is modest, and calling it
    "clinically meaningful" contradicts the source you are citing. "Clinically significant" is a
    term of art: use it only where the source itself does.
  * PRIORITY. Never write "first", "only", "largest", "longest", "highest" or "unique" unless the
    source makes that exact claim. Superlatives do not transfer between studies: a paper describing
    itself as the largest trial has NOT claimed to be the first, and one paper's superlative says
...
LAYOUT_USAGE = {
    ...
    "chart":              "A native, editable CHART of REAL numbers from the source (the strongest way to show a result). Emit `title` (an action title stating the ONE insight), an optional `caption` (a one-line reading of the result), `chart_type`, `categories` (2 to 8 axis labels) and `series` (1 to 4 objects, EACH with BOTH a `name` and a `values` array aligned to the categories — a series with a name but no values, or values but no name, is broken and will fail). `series` is a FIELD INSIDE a chart slide, never a slide's own `layout` — every slide's `layout` must be `chart` (or another real layout name from the list above), NEVER the literal word `series`. If the deck needs a SECOND chart, emit a second, complete, independent slide with `\"layout\": \"chart\"` and its own full `categories`/`series` — do not split one chart's fields across two slides. AXIS TITLES ARE MANDATORY: ALWAYS emit `x_axis` (the category dimension, e.g. 'Study group' or 'Week') AND `y_axis` (what is measured plus its units, e.g. 'CRP reduction (%)', 'Omega-3 index', 'IL-2 (pg/mL)'). Never leave an axis unlabeled. MATCH THE TYPE TO THE DATA: a TREND over time -> 'line'; comparing categories -> 'column' (or 'bar'); PART-TO-WHOLE shares of one total -> 'stacked_100' or 'doughnut'. Do NOT use a doughnut unless it is genuinely parts of one whole. Use ONLY figures explicitly stated in the source; never invent numbers.",
