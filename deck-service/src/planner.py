...
LAYOUT_USAGE = {
    ...
    "timeline":           "A SINGLE-ROW horizontal timeline of 3 to 6 dated milestones. Emit `title` and `milestones`: each with a `date`, a short `heading`, and an optional one-line `body`. Use it ONLY when every milestone is essentially a LABEL — a date plus a few words, with little or no explanatory detail. As soon as the milestones carry a real sentence of substance (a study finding, a dose, an outcome, a result), prefer `alt_timeline` instead: the single row gives each event a narrow column, whereas alt_timeline gives it a proper card. For a dated sequence of STUDIES with findings, `alt_timeline` is almost always the better choice.",
    ...
    "cycle":              "CYCLE: 3 to 6 labelled nodes arranged in a ring around an optional hub (`center`). Emit `title`, optional `center`, and `items`: each a short `heading` (2 to 3 words). Use for a repeating loop, continuous-improvement cycle, or process with no fixed end. Prefer `journey`/`roadmap` for a linear sequence.",
    "serpentine":         "SERPENTINE flow: 3 or 4 stages threaded on an S-curve, numbered discs on the curve and each stage's text alternating above and below it. Emit `title` and `items`: each a `heading` (short — often a 'From X to Y' shift), a one-line `body`, and an `icon` OR `icon_generic` (all items share ONE icon source or none). Use for a narrative sequence of shifts, forces or eras where the FLOW matters. Prefer `journey` for a plain left-to-right process and `from_to` for a single before/after.",
    ...
    "alt_timeline":       "ALTERNATING TIMELINE — the DEFAULT choice for a dated sequence whose events carry real detail (a study timeline with findings, a release history, a schedule of windows). A spine with lettered nodes, event cards placed above and below in turn. Emit `title`, optional `caption`, and `items`: 3 to 7 events each with a `date`, a short `heading`, a one-line `body`, and an optional `tag` (a CATEGORY, e.g. 'Major outage' — repeat the same tag across events of the same kind; distinct tags become a colour legend). Use when events have DATES and categories and there are too many for `timeline` (which is a single row, 3 to 6 milestones).",
    ...
    "gantt":              "GANTT / project schedule: task bars against a time axis. Emit `title`, optional `caption`, `periods` (2 to 8 column labels along the time axis, e.g. ['Q1','Q2','Q3','Q4'] or ['Wk 1','Wk 2',...]), and `items`: 2 to 8 tasks each with a `label`, a `start` (1-based period index where the bar begins), an `end` (1-based period index where it ends; omit for a single-period task), an optional short `note` (printed on the bar), and an optional `milestone: true` (renders a diamond at `start` instead of a bar). Use for a project plan, study schedule, or delivery timeline where tasks have durations across periods. Prefer `roadmap` for a few sequential phases and `timeline` for point-in-time milestones only.",
}
