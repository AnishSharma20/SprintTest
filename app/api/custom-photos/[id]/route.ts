// /api/custom-photos/[id] — edit, toggle or remove one of the team's photos.
//
//   PATCH  { name?, description?, enabled?, author? }
//   DELETE
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { name, description, enabled, author } = (await req.json()) as {
      name?: string;
      description?: string;
      enabled?: boolean;
      author?: string;
    ...
    const upd = await sb
      ...
      .update(patch)
      .eq("id", id)
      .select("id, name, description, enabled, thumb_b64, updated_by, updated_at")
      .single();
