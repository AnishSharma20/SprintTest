...
LAYOUT_USAGE = {
    ...
    "alt_timeline":       "ALTERNATING TIMELINE — use this ONLY when a dated sequence has 5 to 7 events, i.e. too many for the wavy `serpentine` (which caps at 4). For 3 or 4 dated events prefer `serpentine`. A spine with lettered nodes, event cards placed above and below in turn. Emit `title`, optional `caption`, and `items`: 3 to 7 events each with a `date`, a short `heading`, a one-line `body`, and an optional `tag` (a CATEGORY, e.g. 'Major outage' — repeat the same tag across events of the same kind; distinct tags become a colour legend). Use whenever events have DATES — a study timeline with findings, a release history, a schedule of windows.",
...
- a sequence or process, or 3 to 4 dated events -> `serpentine` (the wavy timeline);  5 to 7 dated events -> `alt_timeline`;
