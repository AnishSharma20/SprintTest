...
LAYOUT_USAGE = {
    ...
    "gantt":              "GANTT / project schedule: task bars against a time axis. Emit `title`, optional `caption`, `periods` (2 to 8 column labels along the time axis, e.g. ['Q1','Q2','Q3','Q4'] or ['Wk 1','Wk 2',...]), and `items`: 2 to 8 tasks each with a `label`, a `start` (1-based period index where the bar begins), an `end` (1-based period index where it ends; omit for a single-period task), an optional short `note` (printed on the bar), and an optional `milestone: true` (renders a diamond at `start` instead of a bar). Use for a project plan, study schedule, or delivery timeline where tasks have durations across periods. Prefer `roadmap` for a few sequential phases and `alt_timeline` for dated events without durations.",
}
...
- a sequence or process -> `roadmap` or `serpentine`;  dated events / a study over time -> `alt_timeline`;
