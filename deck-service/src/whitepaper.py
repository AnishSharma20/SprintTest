...
# ===========================================================================
# COMPOSED InDesign whitepaper — pages picked from SEVERAL Superba brochures.
#
# Unlike generate_whitepaper_idml (one fixed template, so every output looks alike), this
# assembles a document from the page library (config/idml_pages.json): Claude first picks an
# ordered page set, then writes text for the fillable pages only. Verbatim pages (benefit grid,
# ingredient/portfolio spread) are placed exactly as designed.
# ===========================================================================
...
def generate_whitepaper_composed(client: anthropic.Anthropic, source_text: str, base_name: str, *,
    ...
    `pages` overrides the automatic selection (the UI's manual page picker)."""
    ...
    for attempt, budget in enumerate((24000, 32000)):
        ...
        # Completeness = every fillable page AND every one of its slots. Checking only the page
        # keys let a page through with 2 of 5 frames written, leaving the rest blank on the page.
        gaps: list[str] = []
